...
{
    // compile ov model
    model = core.compile_model("/home/dhu/DIODE-Vision-2025/src/buff_inference/model/best.xml", "GPU");
    // create ov request
...
void buffInferenceNode::makeTransforms()
{
    ...
    if (!fans.empty()) {
        ...
        // 将相机中心映射到图片上
        cv::projectPoints(centerPoint, rvec_, tvec_, cameraMatrix, distCoeffs, predictPoint);
